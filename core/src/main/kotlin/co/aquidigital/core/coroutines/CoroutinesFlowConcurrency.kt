package co.aquidigital.core.coroutines
 ...
 *
 * More parallel examples can be found in
 * [this](https://github.com/Kotlin/kotlinx.coroutines/issues/1147) issue.
 * To be used unless Kotlin Coroutines team would adapt it to the library (issue opened since 2019).
 *
 *
 * Returns a flow with elements equal to `flatMap(transform)`, including having the same order,
 * but evaluates up to `maxConcurrency` of the flows concurrently, up to a limit of `buffer`
 * flows ahead of the consumer.
 *
 * For example, `flowOf(a, b, c, d, e).flatMapConcurrently
 * (maxConcurrency = 3, buffer = 4, transform)` will
 * evaluate `transform(a)`, `transform(b)`, and `transform(c)` concurrently, and will
 * start evaluating `transform(d)` as soon as one of those complete, but `transform(e)`
 * will not start until `transform(a)` is collected.
 *
 * If `x` is emitted by the backing flow and `transform(x)` throws an exception, the returned flow
 * will attempt to cancel the evaluation of `transform` on any values emitted after `x`, but will
 * continue evaluating `transform` on values emitted before `x`.  So in the above example, if
 * `transform(b)` throws before `transform(a)` or `transform(c)` complete, `transform(c)` will be
 * cancelled but `transform(a)` will be allowed to complete.
 */
...
internal fun <T, R> Flow<T>.flatMapConcurrently(
    ...
    return flow {
        ...
        supervisorScope {
            ...
            while (true) {
                ...
                exceptionWasThrownEarlier.invokeOnCompletion { thrown -> // should never be null
                    ...
                    // it's possible that evalTransform completed successfully,
                    // but there are other downstream transform's to
                    // cancel, so we can't depend on the evalTransform completion handler
                    // to propagate thrown
                    exceptionWasThrownEarlierOrHere.completeExceptionally(thrown)
                }
                emit(evalTransform)
        ...
        //
        // "It is easy to remember. Without buffer there is no concurrency - at most 1
        // operation is executed at any time,
